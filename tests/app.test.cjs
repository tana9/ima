...
test('APIは引数を転送し、失敗時はPromiseを拒否する', async () => {
  ...
  handlers.echo = () => { throw new Error('アクセスが拒否されました'); };
  await assert.rejects(context.callServer('echo'), /アクセスが拒否されました/);
});

test('応答が逆順に届いても最新のダッシュボード取得結果を採用する', async () => {
  ...
  second.resolve({ status: { active: false }, events: [], titles: ['新しい記録'] });
  assert.equal(await newRequest, true);
  first.resolve({ status: { active: true }, events: [], titles: ['古い記録'] });
  assert.equal(await oldRequest, false);
  assert.equal(context.appState.titles[0], '新しい記録');
  assert.equal(context.appState.status.active, false);
  assert.equal(context.appState.loading, false);
});

test('初回読み込み失敗時に再試行を表示し、再試行成功後に操作を有効にする', async () => {
  const { context, handlers, fields, result } = client();
  context.appState.status = null;
  fields.titleInput.value = '作業';
  handlers.getDashboard = () => { throw new Error('オフラインです'); };
...
test('確認中と通信中は開始の二重実行を防ぐ', async () => {
  ...
  request.resolve({ active: true, title: '作業', startTime: Date.now(), eventId: 'id' });
  await first;
...
test('開始失敗時は入力を保持し、再試行できるよう操作ロックを解除する', async () => {
  const { context, handlers, fields, result } = client();
  fields.titleInput.value = '作業';
  fields.descriptionInput.value = 'メモ';
  ...
  assert.equal(fields.descriptionInput.value, 'メモ');
  assert.equal(fields.imageInput.files[0].name, '写真.png');
  assert.equal(context.appState.busy, false);
  assert.equal(fields.startBtn.disabled, false);
  assert.match(result.messages.at(-1).message, /オフラインです/);
});

test('確認を取り消すと更新を送信せず、操作ロックを解除する', async () => {
...
test('終了失敗時は入力日時と進行中の状態を保持する', async () => {
  const { context, handlers, input } = client();
  context.appState.status = { active: true, title: '作業', startTime: 1000 };
  input.value = '2026-09-24T00:20';
  handlers.finishActivity = () => { throw new Error('オフラインです'); };
...
test('日時処理は不正な形式、存在しない日時、不正な期間を拒否する', () => {
  const { context } = client();
...
test('終了日時が開始日時以前なら入力を保持し、確認を表示しない', async () => {
...
test('内容だけの編集では短時間や日付をまたぐ記録の秒を保持する', async () => {
...
test('編集の入力検証で送信を防ぎ、再表示後も下書きを保持する', async () => {
  const { context, result } = client();
  const event = { id: 'id', title: '作業', start: new Date(2026, 8, 23, 10).getTime(),
...
test('添付失敗時は再試行できるよう編集下書きとファイルを保持する', async () => {
  ...
  context.appState.editingEventId = 'id';
  handlers.updateEvent = () => ({ updated: true });
  handlers.attachImageToEvent = () => { throw new Error('アップロードに失敗しました'); };
  handlers.getDashboard = () => ({ status: { active: false }, events: [event], titles: [] });
  context.readImage = async () => 'base64';
  const form = context.buildEditForm(event);
  const file = { name: '写真.png', size: 100, type: 'image/png' };
  ...
  assert.equal(context.buildEditForm(event).querySelector('.edit-image-name').textContent, '写真.png');
});

test('編集中の記録を削除すると編集状態を解除し、画面全体を更新する', async () => {
  const { context, handlers, result } = client();
  const event = { id: 'id', title: '作業', start: 1000, end: 61000, active: true };
  context.appState.editingEventId = 'id';
